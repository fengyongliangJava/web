...
function CTRSCalendar(){
	...
	this.drawCalendarWithTime = CTRSCalendar_drawCalendarWithTime;
...
function CTRSCalendar_drawCalendar(id, sValue, format, showsTime, showsOtherMonths, bNotSubmit, sStyle, bApplyEmpty){
	...
	var sHTML = ""
			+"<input readonly=\"true\" MAXLENGTH=\""+nMaxLength+"\" DateFormat=\""+format+"\" onblur=\"checkDateString(";
	if(bApplyEmpty) {
		sHTML += "true";
	}
	...
	sHTML += ""+showsOtherMonths+");\"><img src=\"img/img.gif\" style=\"cursor: pointer; border: 1px solid red;\""
			+" onmouseover=\"this.style.background='red';\" onmouseout=\"this.style.background=''\" border=\"0\"/></A>";

	document.write(sHTML);
}
function CTRSCalendar_drawCalendarWithTime(id, sValue, format, showsTime, showsOtherMonths, bNotSubmit, sStyle, bApplyEmpty){
	var nMaxLength = 16;
	if(format == "%Y-%m-%d %H:%M") nMaxLength = 16;
	if(format == "%Y-%m-%d") nMaxLength = 10;
	var sHTML = ""
			+"<input MAXLENGTH=\""+nMaxLength+"\" DateFormat=\""+format+"\" onblur=\"checkDateString(";
	if(bApplyEmpty) {
		sHTML += "true";
	}
	sHTML += ")\" class=\"inputtext\" type=\"text\" name=\""+id+"\" id=\""+id+"\" value=\""+sValue+"\"";
...
function CTRSCalendar_drawWithTime(id, sValue, bNotSubmit, bApplyEmpty){
	...
	this.drawCalendarWithTime(id, sValue, '%Y-%m-%d %H:%M', '24', true, bNotSubmit, "width:110px", bApplyEmpty);
}

function CTRSCalendar_drawWithoutTime(id, sValue, bNotSubmit, bApplyEmpty){
	if(sValue==null || sValue=="") {
		var date = new Date();
...
function CTRSCalendar_drawDialogCalendar(id, _sInitDateString, _sDateFormat, _bShowTime, sStyle, bNotSubmit){
	...
	sHTML += ");\"><img src=\"img/img.gif\" style=\"cursor: pointer; border: 1px solid red;\""
			+" onmouseover=\"this.style.background='red';\" onmouseout=\"this.style.background=''\" border=\"0\"/></A>";
