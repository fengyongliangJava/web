		...
		(function(){
			...
			function setValue(oSrcElement, nKeyCode){
				...
				if(!oSrcElement.beOnFocus || oSrcElement.value==""){
					if(!cal.bFlatDisplay)//在DialogCalendar中不需要走这一步复制，有默认的传值方式
			...
			function selectNumber(){
				event.srcElement.beOnFocus = false;
				event.srcElement.select();
			};
			function disabledFunction(){return false;}
			//function beFocusIn(){event.srcElement.beOnFocus = true;};
			function beFocusOut(){
				var oSrcElement = event.srcElement;
				var oElName = oSrcElement.name;
				oSrcElement.beOnFocus = false;
			...
			function makeTimeInput(className, init, range_start, range_end) {
				...
				//Calendar.addEvent(part, "keypress", getNumberInput);
				Calendar.addEvent(part, "mouseover", selectNumber);
				Calendar.addEvent(part, "mouseup", selectNumber);
				//Calendar.addEvent(part, "mouseout", beFocusOut);
...
Calendar.prototype.showAtElement = function (el, opts) {
	...
	function fixPosition(box) {
		...
		var tmp = 0;//box.x + box.width - br.x;
		if (tmp > 0) box.x -= tmp;
		tmp = 0;//box.y + box.height - br.y;
		if (tmp > 0) box.y -= tmp;
	};
	this.element.style.display = "block";
